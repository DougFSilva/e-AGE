...
public class EnrollmentValidator {
	...
	public void studentNotEnrolledInClazz(Student student, Clazz clazz) {
		if(repository.existsByClazzAndStudent(student, clazz)) {
			throw new EnrollmentValidationException(String.format("The student %s is already enrolled in the class %s!", student.getName(), clazz.getCode()));
		}
	}
	
	public void clazzIsNotClosed(Clazz clazz) {
		if(clazz.getIsClosed()) {
			throw new EnrollmentValidationException("It is not possible to enroll a student in a closed class");
		}
	}
}
